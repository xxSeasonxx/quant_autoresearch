...
>
> **Update 2026-07-21 — P2 worthwhile subset applied.** After re-grounding every open P2 item against current source, Season chose the subset that fixes dishonest/inconsistent evidence, a self-inflicted stop hazard, and cheap doc-truth gaps, and skipped the over-built ones. Shipped: **item 18 (F23/F24)** — a hard feasibility breach classes as `failure_class = infeasible` (a distinct economic verdict, not `run_error`), and the ledger row and run card now derive the crash class from one source so they always agree; **item 20 (F26)** — `climb` sources the frozen thesis identity from the lock (identity args are optional), removing the paraphrase-wedge hazard while preserving the lock's changed-identity guard; **item 22 (F28)** — `reset` archives the `results/autoresearch/` attempt tree with the other lifecycle state; **item 22 (F9)** — past-tensed the superseded-scheme lines in `HISTORY.md` that read as current; **item 15 (F12)** — stated the durable no-ranking-haircut rule in `docs/score_research.md`. **Skipped: item 16 (F17)** (read-time ledger↔run-card reconciliation — defends a single operator hand-editing a forbidden file; adds read-path fragility), **item 17 (F18)** (`status` already surfaces incompleteness; "abandoned" is unknowable in an agent-invoked loop), **item 22 (F27)** (flat-subwindow non-scoreable is documented-intended, and the window is already named in the ledger `detail`), **item 22 (F19)** (Season-level OOS governance), **item 22 (F21)** (edits the loaded thesis file, review §11 out-of-scope; the guard is already owned by `program.md`). **item 22 (F20)** needed no change — no stale causality over-kill note remains in the repo.
...
| 15 | Done | P2 | F12 State the durable no-ranking-haircut rule in the active doc | Refactor | `docs/score_research.md` |
| 16 | Skipped | P2 | F17 Reconcile ledger rows against run cards on read (over-built; read-path fragility) | Add | `results_log.py`/`loop.py` |
| 17 | Skipped | P2 | F18 Machine-visible early-stop marker (`status` already shows incompleteness) | Add | `loop.py` |
| 18 | Done | P2 | F23/F24 Distinct `infeasible` class; agree crash class across artifacts | Refactor | `loop.py` |
| 19 | Done | P2 | F25 Test `_stop_reason_after_attempt`; delete dead helpers | Add/Retire | `tests/`/`objective.py` |
| 20 | Done | P2 | F26 Source identity from lock, not re-passed args | Refactor | `loop.py` |
| 21 | Done | P2 | F8 Stall prohibition → gather-diagnostic action | Refactor | `program.md` |
| 22 | Partial | P2 | F9 past-tense HISTORY (Done); F28 reset archives attempt tree (Done); F20 no stale note (n/a); F21 rationale guard, F27 flat-window, F19 tried-universe ledger (Skipped) | Refactor/Add | various |
