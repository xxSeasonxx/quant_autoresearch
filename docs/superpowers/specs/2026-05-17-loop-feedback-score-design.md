...
The score must stay boring:

```text
score = screen_summary.net_return
```
...
- `experiment.yml` remains current loop configuration.
...
validation_failed score = screen_summary.net_return
validated         score = screen_summary.net_return
```
...
- validation-failed attempt writes score from `screen_summary.net_return`
