...
- [x] 2.2 Keep `return_subwindow` as undeflated point-estimate diagnostic; keep PSR/Sharpe/Calmar as diagnostics; reject unknown `objective.kind`.
...
- [x] 3.3 Add causality hard gate: fail when upstream `causality_verified` is false.
...
- [x] 3.5 Gate tests: money floor fails with positive point estimate but deflated LCB below hurdle; retention fails / is non-binding when `R_full(realistic) ≤ 0`; unverified causality fails; gate failure does not change the score.
...
- [ ] 4.2 Raise the causality replay budget in `[output]` (`focused_timeout_seconds`, `focused_probe_limit`) and confirm a legitimate run verifies within budget before enabling the hard gate. **Unresolved:** the real run uses `causality_check = "micro"`, but the raised `focused_*` budget applies to the `focused` mode, not `micro`; micro replay still timed out (`timed_out=true, verified=false`, 5 probes) in a 166s attempt. The micro replay needs its own raised budget (or micro causality investigated) before the hard causality gate is safe to rely on — otherwise it over-kills legitimate books on a non-substantive timeout.
