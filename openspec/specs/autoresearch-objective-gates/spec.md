...
The complexity gate SHALL fail when the candidate exceeds the configured maximum declared signal components or bounded params. Signal components SHALL be cross-checkable against `rationale.md`.

#### Scenario: complexity cap fails
- **WHEN** a candidate declares more components or bounded params than allowed
