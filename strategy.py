...
def _generate_signal_payloads(
    ...
    funding_lookback_events = _positive_int(
        params.get("funding_lookback_events", 3), "funding_lookback_events"
    )
    return_lookback_minutes = _positive_int(
        params.get("return_lookback_minutes", 240), "return_lookback_minutes"
    )
    decision_interval_minutes = _positive_int(
        params.get("decision_interval_minutes", 480), "decision_interval_minutes"
    )
    ...
    min_abs_funding_bps = _non_negative_float(
        params.get("min_abs_funding_bps", 1.0), "min_abs_funding_bps"
    )
    min_abs_return_bps = _non_negative_float(
        params.get("min_abs_return_bps", 25.0), "min_abs_return_bps"
    )
    ...
    require_exit_horizon = _bool_param(
        params.get("require_exit_horizon", False), "require_exit_horizon"
    )
    ...
    required_exit_horizon_bars = max(
        ...
        long_hold_bars,
        high_extension_short_hold_bars
        if high_extension_short_return_bps > 0.0
        else short_hold_bars,
    )
    ...
    for as_of_time in as_of_times:
        ...
        positive_tail = [
            ...
            and _passes_idiosyncratic_return(
                candidate, "short", market_return_bps, min_short_idiosyncratic_return_bps
            )
        ]
        negative_tail = [
            candidate
            for candidate in candidates
            if _passes_long_funding_pressure(
                candidate, _regime_funding_threshold(min_abs_funding_bps, market_return_bps)
            )
            and candidate["return_extension_bps"] <= -min_abs_return_bps
            and abs(candidate["latest_funding_bps"]) >= min_latest_abs_funding_bps
            and _passes_return_z(candidate, min_abs_return_z)
            ...
            and _passes_idiosyncratic_return(
                candidate, "long", market_return_bps, min_long_idiosyncratic_return_bps
            )
        ...
        selected_longs = (
            _selected_tail(negative_tail, "long", selection_score, top_n)
...
def generate_decisions(
    ...
    signals = _generate_signal_payloads(bars, params)
    bars_by_symbol = _symbol_bar_timestamps(bars)
    available_at_by_symbol = _symbol_available_at_by_timestamp(bars)
    by_symbol: dict[str, list[dict[str, object]]] = {}
    for signal in signals:
        by_symbol.setdefault(str(signal["symbol"]), []).append(signal)
    ...
    for symbol in sorted(by_symbol):
        ...
        symbol_signals = sorted(
            by_symbol[symbol],
            key=lambda signal: (
                _as_datetime(signal["decision_time"]),
                _as_datetime(signal["as_of_time"]),
            ),
        )
        for index, signal in enumerate(symbol_signals):
            ...
            signal_weight = _positive_float(signal["weight"], "weight")
            target = signal_weight if side == "long" else -signal_weight
            if target == standing_target or decision_time in emitted_times:
                continue

            observations: tuple[ObservationRef, ...] = tuple(
                cast(Sequence[ObservationRef], signal.get("observations", ()))
            )
            decisions.append(
                TargetDecision(
            ...
            flat_time = _bar_at_offset(
                bars_by_symbol.get(symbol, ()),
                decision_time,
                _positive_int(signal.get("max_hold_bars", signal["hold_bars"]), "max_hold_bars"),
            )
            if flat_time is None:
                continue
            flat_decision_time = _first_bar_at_or_after(
                bars_by_symbol.get(symbol, ()),
                available_at_by_symbol.get(symbol, {}).get(flat_time, flat_time),
            )
            if flat_decision_time is None:
                continue
            next_decision_time = (
                _as_datetime(symbol_signals[index + 1]["decision_time"])
                if index + 1 < len(symbol_signals)
                else None
            )
            superseded = next_decision_time is not None and next_decision_time <= flat_decision_time
            if superseded or flat_decision_time in emitted_times:
                continue
            decisions.append(
                TargetDecision(
                    strategy_id=_STRATEGY_ID,
            ...
            emitted_times.add(flat_decision_time)
            standing_target = 0.0
    return decisions
...
def _validate_scalar_params(params: Mapping[str, object]) -> None:
    ...
    _non_negative_float(
        params.get("max_short_return_extension_bps", 0.0), "max_short_return_extension_bps"
    )
    _bool_param(
        params.get("include_positive_funding_shorts", True), "include_positive_funding_shorts"
    )
    _bool_param(
        params.get("include_negative_funding_longs", True), "include_negative_funding_longs"
    )
    ...
    _non_negative_int(
        params.get("recent_return_lookback_minutes", 0), "recent_return_lookback_minutes"
    )
    _non_negative_float(
        params.get("max_recent_same_direction_return_bps", 0.0),
        "max_recent_same_direction_return_bps",
    ...
    _positive_float(params.get("weight", 1.0), "weight")
    hold_bars = _positive_int(params.get("hold_bars", params.get("hold_minutes", 480)), "hold_bars")
    short_hold_bars = _positive_int(params.get("short_hold_bars", hold_bars), "short_hold_bars")
    _positive_int(params.get("long_hold_bars", hold_bars), "long_hold_bars")
    _non_negative_float(
        params.get("high_extension_short_return_bps", 0.0), "high_extension_short_return_bps"
    )
    _positive_int(
        params.get("high_extension_short_hold_bars", short_hold_bars),
        "high_extension_short_hold_bars",
...
def _risk_rule(signal: Mapping[str, object]) -> RiskRule | None:
    legs: dict[str, float] = {}
    if "stop_loss_bps" in signal:
        stop_loss_bps = _optional_positive_float(signal.get("stop_loss_bps"), "stop_loss_bps")
        if stop_loss_bps is not None:
            legs["stop_loss"] = stop_loss_bps / 1e4
    if "take_profit_bps" in signal:
        take_profit_bps = _optional_positive_float(
            signal.get("take_profit_bps"), "take_profit_bps"
        )
        if take_profit_bps is not None:
            legs["take_profit"] = take_profit_bps / 1e4
    if "trailing_stop_bps" in signal:
        trailing_stop_bps = _optional_positive_float(
            signal.get("trailing_stop_bps"), "trailing_stop_bps"
        )
        if trailing_stop_bps is not None:
            legs["trailing"] = trailing_stop_bps / 1e4
    if not legs:
        return None
...
def _rows_by_symbol(
    ...
    *,
    ...
    rows_by_symbol: dict[
        str, list[tuple[datetime, float | None, datetime | None, float | None, bool]]
    ] = {}
    for row in bars:
        ...
        candidates.append(
            {
                ...
                "recent_return_bps": _recent_return_bps(
                    rows, observed_time, recent_return_lookback_minutes
                ),
                "observation_points": (
                    (observed_time, "close"),
                    (base_time, "close"),
                    *(
                        ...
                        for timestamp in cast(
                            tuple[datetime, ...],
                            funding_stats["funding_observation_timestamps"],
                        )
                    ),
                ),
            }
...
def _recent_return_bps(
    rows: _SymbolRows, observed_time: datetime, lookback_minutes: int
) -> float | None:
...
def _record_active_window(
    ...
    if state_mode == "off":
        return
    active_until_by_symbol[symbol] = decision_time + timedelta(
        minutes=hold_bars + overlap_exit_buffer_bars
    )
...
def _symbol_bar_timestamps(
    ...
    for row in bars:
        by_symbol.setdefault(str(row["symbol"]), set()).add(_as_datetime(row["timestamp"]))
    return {symbol: sorted(timestamps) for symbol, timestamps in by_symbol.items()}


def _symbol_available_at_by_timestamp(
    bars: Sequence[Mapping[str, object]],
) -> dict[str, dict[datetime, datetime]]:
    available_by_symbol: dict[str, dict[datetime, datetime]] = {}
    for row in bars:
        timestamp = _as_datetime(row["timestamp"])
        available_at = _optional_datetime(row.get("available_at"))
        if available_at is None:
            continue
        available_by_symbol.setdefault(str(row["symbol"]), {})[timestamp] = available_at
...
def _is_decision_time(
    ...
    session_start_hour = _non_negative_int(
        params.get("session_start_hour", 0), "session_start_hour"
    )
    session_end_hour = _non_negative_int(params.get("session_end_hour", 24), "session_end_hour")
...
def _passes_symbol_session_start_gate(
    symbol: object, timestamp: datetime, params: Mapping[str, object]
) -> bool:
